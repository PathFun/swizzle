...
export default defineComponent({
  ...
  setup(props) {
    ...
    return () => {
      ...
      const { labelClass = '', labelStyle = {}, schema = {}, displayType = 'column' } = props;
      ...
      const Label = title ? h('label',
          {
            class: `fr-label-title ${isCheckBoxType(schema, readOnly) || _displayType === 'column' ? 'no-colon' : ''}`,
            title
          },
          [
            required === true && h('span', { class: 'fr-label-required' }, ' *'),
            h('span', { class: `${isObjType ? 'b' : ''} ${_displayType === 'column' ? 'flex-none' : ''}` },
