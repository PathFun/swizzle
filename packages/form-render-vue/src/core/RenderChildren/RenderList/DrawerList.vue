...
export default defineComponent({
  ...
  props: {
    ...
    getFieldsProps: {
      type: Function as PropType<(idx: number, extraProps?: object) => FieldsProps>,
      required: true,
      default: () => fnDefault
    }
  },
    ...
    return () => {
      ...
      const columns = childData.map(child => {
        ...
        return {
          ...
          $child: child,
          title: `${schema.title}${required ? '-is-required' : '-is-not-required'}`,
          isRequired: required,
          width: FIELD_LENGTH,
          ...columnProps,
        };
      ...
      return [
        h('div', { class: 'w-100 mb2 tr' }, [
          !props.hideAdd && h(Button, { type: 'primary', size: 'small', onClick: handleAdd }, () => '新增'),
          ...childrenBtn
        ]),
        h(Drawer, {
          width: '600px',
        ...
        h(Table, {
          ...
          columns,
          dataSource,
          rowClassName: (record: { $idx: number, [key: string]: any }, idx: number) => {
          ...
          bodyCell: (cellData: {
            ...
            if (record.key === '$action') {
              return h('div', [
                ...
                !props.hideMove && h(ArrowUpOutlined, {
                  style: {
                    color: '#1890ff',
                    fontSize: '16px',
                    marginLeft: '8px'
                ...
                !props.hideMove && h(ArrowDownOutlined, {
                  style: {
                    color: '#1890ff',
                    fontSize: '16px',
                    marginLeft: '8px'
            ...
            const errorObj: { [key: string]: any } = errorFields.find(item => item.name == childPath) || {};

            return h('div', [
              h('div', {}, getDisplayValue(text, schema)),
              errorObj.error && h(ErrorMessage, { message: errorObj.error, schema })
            ])
          },
          headerCell: (cellData: {
            title: string,
