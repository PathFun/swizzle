...
export default defineComponent({
  ...
  setup(aProps) {
    ...
    const onEdit = (
      targetKey: string | number | KeyboardEvent | MouseEvent,
      action: 'add' | 'remove',
    ) => {
      if (action === 'add') {
    ...
    return () => {
      ...
      const childrenList: any[] = displayList.map((item, idx) => {
        ...
        const currentTabPaneName: any =
          tabName && tabName instanceof Array
            ? tabName[idx] || idx + 1
            : `${tabName || '项目'} ${idx + 1}`;
        return h(
          TabPane,
          { tab: currentTabPaneName, key: `${idx}` },
          {
      ...
      return h(
        Tabs,
        { type: type || 'line', onChange: setActiveKey, onEdit, ...restProps },
        {
