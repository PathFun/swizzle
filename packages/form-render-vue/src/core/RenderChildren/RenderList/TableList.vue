...
export default defineComponent({
  ...
  setup(aProps) {
    return () => {
      ...
      const paginationConfig = pagination && typeof pagination === 'object' && {
        size: 'small',
      ...
      return [
        h('div', { class: 'w-100 mb2 tr' }, [
          !props.hideAdd && h(Button, { type: 'primary', size: 'small', onClick: addItem }, { default: () => '新增' }),
          ...childrenBtn
        ]),
        h(Table, {
          ...
          bodyCell: (cellData: {
            ...
            if (record.key === '$action') {
              return h('div', [
                !props.hideAdd && !props.hideCopy && h('a', { onClick: () => copyItem(idx) }),
                !props.hideDelete && h(Popconfirm, { title: '确定删除?', onConfirm: () => deleteItem(idx), okText: '确定', cancelText: '取消'},
                    h('a', { style: { marginLeft: '8px' } }, '删除')),
                !props.hideMove && h(ArrowUpOutlined, { style: { color: '#1890ff', fontSize: '16px', marginLeft: '8px' }, onClick: () => moveItemUp(idx) }),
                !props.hideMove && h(ArrowDownOutlined, { style: { color: '#1890ff', fontSize: '16px', marginLeft: '8px' }, onClick: () => moveItemDown(idx) }),
              ])
            }
            return h(Core, {
              hideTitle: true,
